...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ accounts: accounts.map((account) => safeAccount(account, getPublicOrigin(request), chats.filter((chat) => chat.channelAccountId === account.id).reduce((sum, chat) => sum + chat.unreadCount, 0))) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ account: safeAccount(account, getPublicOrigin(request)) }, { status: 201 });
  } catch (error) {
    return Response.json({ error: errorMessage(error) }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ account: safeAccount(account, getPublicOrigin(request)), connected: false });
  } catch (error) {
    return Response.json({ error: errorMessage(error) }, { status: 500 });
  }
}
