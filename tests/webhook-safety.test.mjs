...
test("LINE webhook persists once and defers AI work", () => {
  ...
  assert.match(webhook, /ได้รับข้อความแล้วค่ะ กำลังอ่านรายละเอียด/);
  assert.match(webhook, /https:\/\/api\.line\.me\/v2\/bot\/message\/push/);
...
test("Cloudinary assets are isolated by merchant and workspace", () => {
  assert.match(cloudinary, /manychatbot\/\$\{safeSegment\(options\.ownerUserId\)\}\/\$\{safeSegment\(options\.workspaceId\)\}/);
  assert.match(cloudinary, /category: "admin-documents" \| "conversation-media"/);
  assert.doesNotMatch(cloudinary, /options\.folder/);
});
