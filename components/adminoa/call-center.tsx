...
export function CallCenterView({
  ...
  const effectiveSelectedId = conversations.some((item) => item.id === selectedId)
    ? selectedId
    : conversations[0]?.id ?? "";
  const selected = conversations.find((item) => item.id === effectiveSelectedId);
  ...
  useEffect(() => {
    if (!selected?.id || selected.unreadCount <= 0) return;
    const timer = window.setTimeout(() => {
      void fetch("/api/conversations", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: selected.id, markRead: true }),
      }).then((response) => response.ok ? onReload() : undefined).catch(() => undefined);
    }, 0);
    ...
    return (
      <div className="mx-auto w-full max-w-5xl py-8">
        <div className="mb-5 rounded-[24px] border border-emerald-100 bg-emerald-50 p-5">
          <div className="flex items-center gap-3"><Route className="size-6 text-emerald-800" /><div><h1 className="font-black text-emerald-950">LINE OA → กล่องกลาง → AI Router</h1><p className="mt-1 text-sm text-emerald-800">ทุกคำถามจะถูกวิเคราะห์และส่งให้ Admin AI ที่มี Skill ตรงที่สุด หากไม่ตรงจะรอพนักงานจริง</p></div></div>
        </div>
        <EmptyState icon={Headphones} title="ยังไม่มีข้อความจาก LINE OA" detail="เมื่อตั้งค่า AI และเชื่อม Webhook ของ LINE OA สำเร็จ ชื่อลูกค้า คำถาม ผู้ตอบ และสถานะส่งต่อจะปรากฏในหน้านี้อัตโนมัติ" />
    ...
    <div className="flex min-h-0 flex-1 overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_16px_45px_rgba(15,23,42,0.08)]">
      <section className={`${mobileThread ? "hidden md:flex" : "flex"} w-full min-w-0 flex-col border-r border-slate-200 md:w-[330px] md:shrink-0 lg:w-[380px]`}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {visible.map((item) => (
            <button key={item.id} onClick={() => { setSelectedId(item.id); setMobileThread(true); }} className={`flex w-full gap-3 border-b border-slate-100 p-4 text-left transition ${selected?.id === item.id ? "bg-emerald-50/80" : "hover:bg-slate-50"}`}>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2"><p className="truncate text-sm font-bold text-slate-900">{item.customerName}</p><span className="ml-auto shrink-0 text-xs text-slate-400">{formatDateTime(item.lastMessageAt)}</span></div>
                <p className="mt-1 line-clamp-2 text-sm leading-5 text-slate-600">{item.lastCustomerMessage || item.lastMessagePreview}</p>
                <div className="mt-2 flex items-center gap-2">{item.humanTakeover ? <Badge className="border-0 bg-amber-100 text-amber-800"><UsersRound /> {item.humanAgentName || "รอพนักงาน"}</Badge> : item.adminName ? <Badge className="border-0 bg-indigo-50 text-indigo-700"><BrainCircuit /> {item.adminName}</Badge> : <StatusBadge status={item.status} />}{item.unreadCount > 0 && <span className="ml-auto flex size-5 items-center justify-center rounded-full bg-emerald-700 text-xs font-black text-white">{item.unreadCount}</span>}</div>
              </div>
            </button>
          ))}
      ...
      {selected && <aside className="hidden w-[320px] shrink-0 border-l border-slate-200 xl:flex xl:flex-col">
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="rounded-2xl bg-slate-950 p-4 text-white"><div className="flex items-center gap-2 text-sm font-bold"><Clock3 className="size-4 text-emerald-300" /> แชตล่าสุด</div><p className="mt-2 text-xs text-slate-300">{formatDateTime(selected.lastMessageAt)}</p><p className="mt-3 break-all text-xs text-slate-500">LINE user: {selected.externalUserId}</p></div>
        </div>
      </aside>}
    </div>
  );
